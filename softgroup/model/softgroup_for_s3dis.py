...
class SoftGroup(nn.Module):
    ...
    def forward_test(self, batch_idxs, voxel_coords, p2v_map, v2p_map, pniv, coords_float, feats,
        ...
        # boziadd>>> 当验证Area2时，可以检查设置
        #print("lvl_fusion:{}".format(lvl_fusion))
        # boziadd<<<
    ...
    def forward_grouping(self,
        ...
        for class_id in range(self.semantic_classes):
            ...
            #'''
            if class_id in [2, 3, 5, 10]:
            	a = 1
            elif class_id in [7]:   # chair取1.2比较好
                a = 1.2
            elif class_id in [6]:   # door取0.1比较好（0似乎更好），door与windows有牵连，因为将door的偏移系数调整时，会同时影响到door和windows的效果
                a = 0.1
            elif class_id in [9]:
                a = 0.7
            elif class_id in [11]:
            	a = 1
            elif class_id in [12]:  # 将clustter调小后，board和bookcase的指标下降了，设置成0.6，则一些类有很小的波动，而beam的指标上升了
                a = 0.6
            else:
            	a = 0.5
            #'''
