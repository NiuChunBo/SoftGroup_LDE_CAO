...
# boziadd<<<
    ...
    def forward_grouping(self,
        ...
        for class_id in range(self.semantic_classes):
            ...
            pt_pred_max = torch.max(torch.norm(pt_offsets, p=2, dim=0))
            pt_truth_max = torch.max(torch.norm(self.pt_offset_labels_selfAdd, p=2, dim=0))
            print("pt_pred_max:{}\tpt_truth_max:{}".format(pt_pred_max, pt_truth_max))

            pt_pred_min = torch.min(torch.norm(pt_offsets, p=2, dim=0))
            pt_truth_min = torch.min(torch.norm(self.pt_offset_labels_selfAdd, p=2, dim=0))
            print("pt_pred_min:{}\tpt_truth_min:{}".format(pt_pred_min, pt_truth_min))
            #'''
            #a = 1
            #print(a)
            ...
            if class_id == 9:
                ...
                pcd.save_txt_from_list(coords=coords_, pniv=pniv)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_ + pt_offsets_, pniv=pniv)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_ + 0.5*pt_offsets_, pniv=pniv)
                pcd.read_and_show()
