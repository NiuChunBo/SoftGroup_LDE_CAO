...
class ReadXYZRGB():
    def __init__(self, path=None, coords=None, feats=None): 
        super().__init__()
        self.path = path
        self.coords = coords
        self.feats = feats
    ...
    def save_txt_from_list(self, coords=None, feats=None, pniv=None, mode='w'):
        coords = coords.cpu().detach()
        l = np.array(coords).shape[0]
        s = np.array(coords).shape[1]
        if feats == None:
            feats = torch.ones(l, 3)
            feats[0,:] = feats[0,:] * 1
            feats[1,:] = feats[1,:] * 0
            feats[2,:] = feats[2,:] * 0.2
        ...
        # 体素
        if pniv == None:
            feats = (np.array(feats* 0.5)).tolist()
        else:
            print("pniv:{}".format(pniv))
            intensity = np.array(torch.sigmoid(pniv))
            intensity = (intensity * 0.4  ).reshape(-1,1)
            feats = (1-feats * intensity).tolist()
            
        print('s:{}'.format(s))

        print("feats:{}".format(np.array(feats).shape))
        if s == 3:
            self.data = np.concatenate([np.array(coords), feats], axis=1)
        else:
            self.data = np.concatenate([coords, feats], axis=1)[:,1:] 

        print('data:{}'.format(self.data.shape))
        with open(self.path, mode) as tmp:
            print('{}:{}'.format(self.path, tmp))
            for d in self.data:
                s = str(d[0]) + ' ' + str(d[1]) + ' ' + str(d[2]) + ' ' + str(d[3]) + ' ' + str(d[4]) + ' ' + str(d[5]) + '\n'
                tmp.writelines(s)
            tmp.close()       

        print('注意：文件{}已成功保存'.format((self.path)))
    
    # 需要把list数据(path)转化成文件，然后将文件路径保存到path
    def read_and_show(self):
        pcd = o3d.io.read_point_cloud(self.path, format='xyzrgb')
...
class SoftGroup(nn.Module):
    def __init__(self,
        ...
        # boziadd>>>
        self.p2v_map_selfAdd = None
        self.pt_offset_labels_selfAdd = None
        #self.point_number_per_voxel = None
        self.with_original_point_number = True
        if self.with_original_point_number:
            in_channels += 1
            self.in_channels += 1
        # boziadd<<<
    ...
    @cuda_cast
    def forward_train(self, batch_idxs, voxel_coords, p2v_map, v2p_map, pniv, coords_float, feats,    # boziadd
        ...
        # boziadd>>>
        self.p2v_map_selfAdd = p2v_map
        self.pt_offset_labels_selfAdd = pt_offset_labels
        #print("voxel_coord:{}".format(torch.min(voxel_coords, dim=0)))
        # the first situation
        #print("softgroup.py p2v_map:\t{}".format(np.array(p2v_map.cpu().detach()).shape))
        # print("pniv:{}\tpniv.len:{}\tmax(pniv):{}".format(pniv, np.array(pniv.cpu().detach()).shape, max(np.array(pniv.cpu().detach()))))
        #print("softgroup.py v2p_map:\t{}".format(np.array(v2p_map.cpu().detach()).shape))
        # print("voxel_feats:\t{}".format(np.array(voxel_feats.cpu().detach()).shape))
        #self.point_number_per_voxel = [len(p2v_map_row.nonzero().view(-1)) for p2v_map_row in p2v_map]
        #print("point_number_per_voxel:\t{}".format(np.array(self.point_number_per_voxel).shape))
        if self.with_original_point_number:
            voxel_feats = torch.cat((voxel_feats, pniv.view(-1, 1)), 1)
            # normalization
            # a = self.point_number_per_voxel
            # self.point_number_per_voxel = torch.Tensor([(x-min(a))/(max(a)-min(a)) for x in a])
            # concatenate
            #self.point_number_per_voxel = torch.Tensor(self.point_number_per_voxel)
            #voxel_feats = torch.cat((voxel_feats, self.point_number_per_voxel.view(-1,1).cuda()), 1)
        
        #print("voxel_feats:\t{}".format(np.array(voxel_feats.cpu().detach()).shape))
        #print("point_number_per_voxel:\t{}".format((np.array(self.point_number_per_voxel))))
        #print("max point_number_per_voxel:\t{}".format(max(np.array(self.point_number_per_voxel))))
        #print("min point_number_per_voxel:\t{}".format(min(np.array(self.point_number_per_voxel))))
        #print("watch voxel_feats:\t{}".format(voxel_feats[0]))
        # boziadd<<<
        ...
        # boziadd>>>
        # scores_bozi = semantic_scores.softmax(dim=-1)
        # for voxel in p2v_map:
        #     voxel_idx = voxel[1:].nonzero().view(-1)
        #     voxel = voxel[voxel_idx]
        #     #print("voxel:{}".format(voxel[1:]))
        #     for point in voxel[1:]:
        #         print("point:{}".format(point))
        #         print("p:{}\np*s:{}\npt_pred:{}\npt_trurt:{}\ts:{}\ti:{}\nscore:{}".format(coords_float[point].cpu().detach().tolist(), 
        #         (coords_float[point]*50).cpu().detach().tolist(), pt_offsets[point], pt_offset_labels[point], semantic_labels[point], 
        #         instance_labels[point], scores_bozi[point].cpu().detach()))
        #         print("*********************************************************************************************")
        #     print("vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvend")
        #print(semantic_scores.softmax(dim=-1))
        # boziadd<<<
        ...
        if not self.semantic_only:
            proposals_idx, proposals_offset = self.forward_grouping(semantic_scores, pt_offsets,
                                                                    ...
                                                                    voxel_coords,# boziadd
                                                                    instance_labels,# boziadd
                                                                    semantic_labels,# boziadd
                                                                    pt_offset_labels,#boziadd
                                                                    self.grouping_cfg)
    ...
    def point_wise_loss(self, semantic_scores, pt_offsets, semantic_labels, instance_labels,
        ...
        if pos_inds.sum() == 0:
            ...
            offset_loss = F.l1_loss(pt_offsets[pos_inds], pt_offset_labels[pos_inds], reduction='sum') / pos_inds.sum()
    ...
    @cuda_cast
    def forward_test(self, batch_idxs, voxel_coords, p2v_map, v2p_map, pniv, coords_float, feats,
...
# boziadd<<<
    ...
    def forward_grouping(self,
        ...
        for class_id in range(self.semantic_classes):
            ...
            pt_offset_labels_ = self.pt_offset_labels_selfAdd[object_idxs]
            # boziadd>>>
            #a = scores[object_idxs].view(-1,1)
            # print("a:{}\tpt_offsets_:{}".format(np.array(a.cpu().detach()).shape, np.array(pt_offsets_.cpu().detach()).shape))
            #coords_shifted_ = coords_ + pt_offsets_ * a
            # boziadd<<<
            if with_pyramid:
                ...
                if level > 1 or not lvl_fusion:
                    coords_, pt_offsets_, batch_idxs_, l2p_map, pniv, p2l_map = self.pyramid_map(
                        coords_, pt_offsets_, batch_idxs_, level, base_size)
                    # coords_shifted_, pt_offsets_, batch_idxs_, l2p_map = self.pyramid_map(
                    #     coords_shifted_, pt_offsets_, batch_idxs_, level, base_size)
            batch_offsets_ = self.get_batch_offsets(batch_idxs_, batch_size)
            
            # boziadd>>>
            print("p2v_map:{}".format(self.p2v_map_selfAdd[0]))
            print("p2l_map:{}".format(p2l_map[0]))


            #'''
            if class_id == 2 or class_id == 5 or class_id == 7 or class_id == 10:
            	a = 1
            else:
            	a = 0.5
            print("class_id:{}\tlevel:{}\ta:{}\tnum_points:{}".format(class_id, level, a, num_points))
            #'''
            #a = 1
            #print(a)
            # # 可视化体素内的点数分布，体素内的点数越多，那么颜色就越深
            if class_id == 9:
                pcd = ReadXYZRGB("tmp.txt")
                print("pniv > 0.0")
                pcd.save_txt_from_list(coords=coords_)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_ + pt_offsets_)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_ + 0.5*pt_offsets_)
                pcd.read_and_show()

            # 可视化pniv较多的体素，看其是否代表骨架
            coords_bone_1 = None
            coords_bone_2 = None
            coords_bone_3 = None
            coords_bone_4 = None
            pt_offset_bone1 = None
            pt_offset_bone2 = None
            pt_offset_bone3 = None
            pt_offset_bone4 = None

            coords_bone_5 = None
            coords_bone_6 = None
            coords_bone_7 = None
            coords_bone_8 = None
            pt_offset_bone5 = None
            pt_offset_bone6 = None
            pt_offset_bone7 = None
            pt_offset_bone8 = None

            pniv_min = torch.min(pniv, dim=0)[0]
            pniv_max = torch.max(pniv, dim=0)[0]
            print("pniv_min:{}\tpniv_max:{}".format(pniv_min, pniv_max))
            for k, v in enumerate(pniv.cpu().detach()):
                pniv_ratio = (v - pniv_min) / (pniv_max - pniv_min)
                if pniv_ratio >= 0.1:
                    if coords_bone_1 == None:
                        coords_bone_1 = coords_[k].unsqueeze(0)
                    else:
                        coords_bone_1 = torch.cat((coords_bone_1, coords_[k].unsqueeze(0)), dim=0)

                    if pt_offset_bone1 == None:
                        pt_offset_bone1 = coords_[k].unsqueeze(0)
                    else:
                        pt_offset_bone1 = torch.cat((pt_offset_bone1, pt_offsets_[k].unsqueeze(0)), dim=0)

                if pniv_ratio >= 0.2:
                    if coords_bone_2 == None:
                        coords_bone_2 = coords_[k].unsqueeze(0)
                    else:
                        coords_bone_2 = torch.cat((coords_bone_2, coords_[k].unsqueeze(0)), dim=0)

                    if pt_offset_bone2 == None:
                        pt_offset_bone2 = coords_[k].unsqueeze(0)
                    else:
                        pt_offset_bone2 = torch.cat((pt_offset_bone2, pt_offsets_[k].unsqueeze(0)), dim=0)
                
                if pniv_ratio >= 0.3:
                    if coords_bone_3 == None:
                        coords_bone_3 = coords_[k].unsqueeze(0)
                    else:
                        coords_bone_3 = torch.cat((coords_bone_3, coords_[k].unsqueeze(0)), dim=0)

                    if pt_offset_bone3 == None:
                        pt_offset_bone3 = coords_[k].unsqueeze(0)
                    else:
                        pt_offset_bone3 = torch.cat((pt_offset_bone3, pt_offsets_[k].unsqueeze(0)), dim=0)
                
                if pniv_ratio >= 0.4:
                    if coords_bone_4 == None:
                        coords_bone_4 = coords_[k].unsqueeze(0)
                    else:
                        coords_bone_4 = torch.cat((coords_bone_4, coords_[k].unsqueeze(0)), dim=0)

                    if pt_offset_bone4 == None:
                        pt_offset_bone4 = coords_[k].unsqueeze(0)
                    else:
                        pt_offset_bone4 = torch.cat((pt_offset_bone4, pt_offsets_[k].unsqueeze(0)), dim=0)



                if pniv_ratio >= 0.0 and pniv_ratio < 0.1:
                    if coords_bone_5 == None:
                        coords_bone_5 = coords_[k].unsqueeze(0)
                    else:
                        coords_bone_5 = torch.cat((coords_bone_5, coords_[k].unsqueeze(0)), dim=0)

                    if pt_offset_bone5 == None:
                        pt_offset_bone5 = coords_[k].unsqueeze(0)
                    else:
                        pt_offset_bone5 = torch.cat((pt_offset_bone5, pt_offsets_[k].unsqueeze(0)), dim=0)


                if pniv_ratio >= 0.1 and pniv_ratio < 0.2:
                    if coords_bone_6 == None:
                        coords_bone_6 = coords_[k].unsqueeze(0)
                    else:
                        coords_bone_6 = torch.cat((coords_bone_6, coords_[k].unsqueeze(0)), dim=0)

                    if pt_offset_bone6 == None:
                        pt_offset_bone6 = coords_[k].unsqueeze(0)
                    else:
                        pt_offset_bone6 = torch.cat((pt_offset_bone6, pt_offsets_[k].unsqueeze(0)), dim=0)
                
                
                if pniv_ratio >= 0.2 and pniv_ratio < 0.3:
                    if coords_bone_7 == None:
                        coords_bone_7 = coords_[k].unsqueeze(0)
                    else:
                        coords_bone_7 = torch.cat((coords_bone_7, coords_[k].unsqueeze(0)), dim=0)

                    if pt_offset_bone7 == None:
                        pt_offset_bone7 = coords_[k].unsqueeze(0)
                    else:
                        pt_offset_bone7 = torch.cat((pt_offset_bone7, pt_offsets_[k].unsqueeze(0)), dim=0)


                if pniv_ratio >= 0.3 and pniv_ratio < 0.4:
                    if coords_bone_8 == None:
                        coords_bone_8 = coords_[k].unsqueeze(0)
                    else:
                        coords_bone_8 = torch.cat((coords_bone_8, coords_[k].unsqueeze(0)), dim=0)

                    if pt_offset_bone8 == None:
                        pt_offset_bone8 = coords_[k].unsqueeze(0)
                    else:
                        pt_offset_bone8 = torch.cat((pt_offset_bone8, pt_offsets_[k].unsqueeze(0)), dim=0)


            if class_id == 9:
                pcd = ReadXYZRGB("tmp.txt")
                print("pniv>0.1")
                pcd.save_txt_from_list(coords=coords_bone_1)
                pcd.read_and_show()
                

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_1 + pt_offset_bone1)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_1 + pt_offset_bone1 * 0.5)
                pcd.read_and_show()
                

                pcd = ReadXYZRGB("tmp.txt")
                print("pniv>0.2")
                pcd.save_txt_from_list(coords=coords_bone_2)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_2 + pt_offset_bone2)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_2 + pt_offset_bone2 * 0.5)
                pcd.read_and_show()

                
                pcd = ReadXYZRGB("tmp.txt")
                print("pniv>0.3")
                pcd.save_txt_from_list(coords=coords_bone_3)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_3 + pt_offset_bone3)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_3 + pt_offset_bone3 * 0.5)
                pcd.read_and_show()


                pcd = ReadXYZRGB("tmp.txt")
                print("pniv>0.4")
                pcd.save_txt_from_list(coords=coords_bone_4)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_4 + pt_offset_bone4)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_4 + pt_offset_bone4 * 0.5)
                pcd.read_and_show()
                ...
                pcd = ReadXYZRGB("tmp.txt")
                print("0.0 < pniv < 0.1")
                pcd.save_txt_from_list(coords=coords_bone_5)
                pcd.read_and_show()
                

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_5 + pt_offset_bone5)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_5 + pt_offset_bone5 * 0.5)
                pcd.read_and_show()
                

                pcd = ReadXYZRGB("tmp.txt")
                print("0.1 < pniv < 0.2")
                pcd.save_txt_from_list(coords=coords_bone_6)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_6 + pt_offset_bone6)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_6 + pt_offset_bone6 * 0.5)
                pcd.read_and_show()

                
                pcd = ReadXYZRGB("tmp.txt")
                print("0.2 < pniv < 0.3")
                pcd.save_txt_from_list(coords=coords_bone_7)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_7 + pt_offset_bone7)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_7 + pt_offset_bone7 * 0.5)
                pcd.read_and_show()


                pcd = ReadXYZRGB("tmp.txt")
                print("0.3 < pniv < 0.4")
                pcd.save_txt_from_list(coords=coords_bone_8)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_8 + pt_offset_bone8)
                pcd.read_and_show()

                pcd = ReadXYZRGB("tmp.txt")
                pcd.save_txt_from_list(coords=coords_bone_8 + pt_offset_bone8 * 0.5)
                pcd.read_and_show()

                
                
            # for v_idx,voxel in enumerate(p2l_map):
            #     print("level:{}\tclass_id:{}".format(level, class_id))
            #     voxel_idx = voxel[1:].nonzero().view(-1)
            #     voxel = voxel[voxel_idx]
            #     #print("voxel:{}".format(voxel[1:]))
            #     pt_mean = pt_offsets[v_idx]
            #     print("pt_mean:{}".format(pt_mean))
            #     for point in voxel[1:]:
            #         point_o = pt_offsets[point]
            #         # 在这里计算每个点与真值偏移向量之间的皮尔逊相关系数
            #         p_coefficients = self.corrcoef(point_o, self.pt_offset_labels_selfAdd[point])
            #         p_coefficients_m = self.corrcoef(point_o, pt_mean)
            #         print("point_o:{}\tpt_offset_labels:{}".format(point_o, self.pt_offset_labels_selfAdd[point]))
            #         print("p_coefficients:{}\tp_corfficients_m:{}".format(p_coefficients, p_coefficients_m))
            #         print("pt_length_pred:{}\tpt_length_truth:{}".format(torch.norm(pt_offsets[point], p=2, dim=0), torch.norm(pt_offset_labels[point], p=2, dim=0)))
            ...
            #         print("point:{}".format(point))
            #         print("p:{}\np*s:{}\npt_pred:{}\npt_trurt:{}\ts:{}\ti:{}\nscore:{}".format(coords_float[point].cpu().detach().tolist(), 
            #         (coords_float[point]*50/level).cpu().detach().tolist(), pt_offsets[point], pt_offset_labels[point], semantic_labels[object_idxs][point], 
            #         instance_labels[object_idxs][point], semantic_scores[point].cpu().detach()))
            #         print("*********************************************************************************************")
            #     print("vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvend")
            # boziadd<<<
    ...
    def pyramid_map(self, coords_float, pt_offsets, batch_idxs, level=1, base_size=0.02):
        ...
        coords, l2p_map, p2l_map, pniv = voxelization_idx(coords.cpu(), batch_idxs[-1].item() + 1)
        # boziadd<<<
        ...
        return coords_float, pt_offsets, batch_idxs, l2p_map, pniv, p2l_map  # boziadd: pniv, p2l_map
    
    def pyramid_map_selfAdd(self, coords_float, pt_offsets, batch_idxs, level=1, base_size=0.02):
        coords = (coords_float / (base_size * level)).long()
        coords = torch.cat([batch_idxs[:, None], coords], dim=1)
    ...
    def get_instances(self,
        ...
        for i in range(self.instance_classes):
            if i in self.sem2ins_classes:
                ...
                # boziadd>>>
                mask_inds = mask_inds.cpu()
                # print("mask_pred:\t{}".format(mask_pred))
                # print("proposals_idx:\t{}".format(proposals_idx))
                # print("mask_inds:\t{}".format(mask_inds))
                # boziadd<<<
    ...
    def clusters_voxelization(self,
        ...
        # boziadd>>>
        out_coords, inp_map, out_map, pniv = voxelization_idx(coords, int(clusters_idx[-1, 0]) + 1)
        # boziadd<<<
