...
# boziadd>>> 这个函数用来使用C++计算双重体素化映射的第二重映射统计得到的pniv的对应的真实的点云个数，而不是体素个数，因为第二重里是第一重的体素个数，使用此函数将第二重也对应到点云个数
# class Voxelization_PNIV(Function):

#     @staticmethod
#     def forward(ctx, p2l_map, first_pniv):
#         '''
#         :param ctx:
#         :param p2l_map: long (M2, maxActive)
#         :param first_pniv: M1   # M1是第一次体素化映射得到的体素个数
#         :return: second_pniv: int M2 # M2是第二次体素化映射得到的体素个数
#         '''
#         assert p2l_map.is_contiguous()
#         M2 = p2l_map.size(0)
        
#         second_pniv = torch.IntTensor(M2).zero_()
#         ops.voxelization_pniv(p2l_map, first_pniv, second_pniv)
#         return second_pniv
...
# voxelization_pniv = Voxelization_PNIV.apply
# boziadd<<<
...
class Voxelization_Idx(Function):
    ...
    def forward(ctx, coords, batchsize, mode=4):
        ...
        # pniv = torch.IntTensor(N).zero_()  # boziadd
        pniv = torch.IntTensor(N,).zero_()  # boziadd

        ops.voxelize_idx(coords, output_coords, input_map, output_map, batchsize, mode, pniv)   # boziadd
        #print("pniv:\t{}\tpniv.len:\t{}\tmax(pniv):\t{}".format(pniv, np.array(pniv).shape, max(np.array(pniv)))) # boziadd
        point_idxs = pniv.nonzero().view(-1)
        pniv = pniv[point_idxs]
        #print("pniv:\t{}\tpniv.len:\t{}\tmax(pniv):\t{}".format(pniv, np.array(pniv).shape, max(np.array(pniv)))) # boziadd

        return output_coords, input_map, output_map, pniv   # boziadd
