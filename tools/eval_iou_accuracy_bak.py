...
for i in [5]:
    file_name = '/wdc/ncb-old/SoftGroup_multi_versions_manage/SoftGroup/results_for_6fold/log{' \
                '}_test/output_filelist.txt'.format(i)

    # file_name = '/wdc/ncb-old/SoftGroup_original/results/log{' \
    #             '}_test/output_filelist.txt'.format(i)
...
pred_label_filenames = [f.rstrip('_pred.txt') + '_pred_label.txt' for f in pred_data_label_filenames]
...
for i in range(num_room):
    ...
    # pred_ins = data_label[:, -1].reshape(-1).astype(np.int_)
    pred_sem = data_label[:, -2].reshape(-1).astype(np.int_)
    gt_label = np.loadtxt(gt_label_filenames[i])
    gt_ins = gt_label[:, -1].reshape(-1).astype(np.int_)
    gt_sem = gt_label[:, -2].reshape(-1).astype(np.int_)
    pred_label = np.loadtxt(pred_label_filenames[i])
    pred_ins = [-100 if i == -1 else i % 1000 for i in pred_label.astype(np.int_).tolist()]
    # pred_sem = [-100 if i == -100 else int(i / 1000 - 1) for i in pred_label.astype(np.int_).tolist()]
