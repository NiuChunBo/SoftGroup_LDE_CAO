...
from softgroup.evaluation import (PanopticEval, ScanNetEval, evaluate_offset_mae, S3DISEval,
...
from softgroup.util import (collect_results_cpu, get_dist_info, get_root_logger, init_dist,
                            is_main_process, load_checkpoint, rle_decode, rle_encode)  # boziadd>>> rle_encode
...
def main():
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            if cfg.data.test.type == "s3dis" or "stpls3d":
            ...
            # result = model(batch)
            end_time = time.time()
            run_time_per_scan = end_time - start_time
            run_time_per_scans.append(run_time_per_scan)
            # print("i:{} run_time:{:.2f}".format(i, run_time_per_scan))
            # boadd<<<
            results.append(result)
            progress_bar.update(world_size)
        print("max_run_time:{:.2f} ave_run_time:{:.2f}".format(max(run_time_per_scans), sum(run_time_per_scans) / len(run_time_per_scans)))
        progress_bar.close()
    ...
    if is_main_process():
        ...
        if 'instance' in eval_tasks:
            ...
            pred_insts_bak = [[item for item in sublist if item['conf'] >= 0.24] for sublist in pred_insts]

            s3dis_eval.evaluate(pred_insts_bak, sem_labels, inst_labels)
            # boadd<<<
