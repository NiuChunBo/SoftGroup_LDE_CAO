...
def train(epoch, model, optimizer, scaler, train_loader, cfg, logger, writer):
    ...
    for i, batch in enumerate(train_loader, start=1):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.fp16):
            ...
            # print("train.py::loss:{}\tlog_vars:{}".format(loss, log_vars))
        ...
        # boziadd<<<
            optimizer.zero_grad()
            #print("train.py::log_vars:{}\tloss:{}".format(log_vars, loss))
            #print(1)
            scaler.scale(loss).backward()
            #print(2)
            #scaler.scale(loss).backward(retain_graph=True)  # 在此处将计算图保存下来，以保证能够多次反向传播，这对于rise_linear是需要对每一个语义簇都进行一次反向传播的。
