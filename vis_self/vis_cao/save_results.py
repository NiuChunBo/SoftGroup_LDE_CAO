...
files_name = os.listdir('./vis_self/results_for_pniv_and_complete_shift/gt_instance/')
...
for room_name in files_name:
    ...
    for task in tasks:
        subprocess.run(['python ./tools/visualization_bak.py --prediction_path \
        ./vis_self/results_for_pniv_and_complete_shift/ --task ' + task +
                        ' --out ./vis_self/my_pniv_complete_results/' + task + '/' + room_name + '_'
                        + task + '.ply --room_name ' + room_name], shell=True)
        print('saving room: {} task: {}'.format(room_name, task))

# 检查是否全部保存
for task in tasks:
    save_files_name = os.listdir('./vis_self/my_pniv_complete_results/' + task + '/')
