...
def run_program1():
    ...
    subprocess.call(['python', './vis_cao_shift_coord_gt.py', scan_files[scan_id]])


# 定义第二个程序的执行函数
def run_program2():
    subprocess.call(['python', './vis_original_pred.py', scan_files[scan_id]])


# 定义第三个程序的执行函数
def run_program3():
    subprocess.call(['python', './vis_pniv_pred.py', scan_files[scan_id]])
...
# 定义第三个程序的执行函数
def run_program5():
    subprocess.call(['python', './vis_cao_pred.py', scan_files[scan_id]])
...
# 定义第三个程序的执行函数
def run_program7():
    subprocess.call(['python', './vis_complete_shift_coord_gt.py', scan_files[scan_id]])


# 定义第三个程序的执行函数
def run_program8():
    subprocess.call(['python', './vis_sem_gt.py', scan_files[scan_id]])


# 定义第三个程序的执行函数
def run_program9():
    subprocess.call(['python', './vis_original_sem_pred.py', scan_files[scan_id]])


# 定义第三个程序的执行函数
def run_program10():
    subprocess.call(['python', './vis_pniv_sem_pred.py', scan_files[scan_id]])
...
def prev_button_clicked():
    ...
    if scan_id <= 0:
        ...
        print(scan_id)
        text_label.config(
            text=scan_files[scan_id].rstrip('_instance_gt.ply') + '\n' + str((scan_id + 1)) + '/' + str(scan_num))
...
def show_button_clicked():
    ...
    if scan_id < 0 or scan_id > 67:
        ...
        # executor.submit(run_program8)
        # executor.submit(run_program9)
        # executor.submit(run_program10)
...
if __name__ == '__main__':
    # 指定场景文件的根目录
    instance_gt_files_path = f"../my_results/instance_gt/"
    # 获取场景的文件列表
    scan_files = os.listdir(instance_gt_files_path)
    scan_num = len(scan_files)
    scan_id = -1

    print([scan_file.rstrip('.txt') for scan_file in scan_files])
    # 创建窗口
    window = tk.Tk()
