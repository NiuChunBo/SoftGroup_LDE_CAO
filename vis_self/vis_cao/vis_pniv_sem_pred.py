...
def process_file(scan_path):
    pcd = o3d.io.read_point_cloud('../pniv_ablation_results/semantic_pred/' + scan_path.rstrip('_instance_gt.ply') + '_semantic_pred.ply')
    ...
    o3d.visualization.draw_geometries([pcd], 'semantic_pniv_pred of ' + scan_path.rstrip('_instance_gt.ply'), 1920, 1080, 0, 0)
...
if __name__ == "__main__":
    ...
    # 使用 "file_path" 参数
    print("File path:", '../pniv_ablation_results/semantic_pred/' + args.file_path.rstrip('_instance_gt.ply') + '_instance_pred.ply')
    # 获取命令行参数
    if len(sys.argv) > 1:
